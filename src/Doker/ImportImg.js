...
import { geNetworkData, CreateNetwork,ImportImage,ContainerList,ListofImages } from '../Redux/Auth/action'//APi Redux function import
import "./header.css";
import axios from 'axios';


export const Home = ({ geNetworkData, Networklist, CreateNetwork,importImg,containerList,
  ContainerList,
  ...
  useEffect(() => {
    ...
    //   geNetworkData().then((payload) => {
    //     setnetworkData(payload);
    //   })
    //   debugger
    // })

    // fetch("http://54.210.126.34:2375/networks").then(
...
console.log(filteredData,"filteredData");
  ...
  const handlecontainer = (option, index) => {
    ...
    setIsDropdownOpen(false)
    const filteredData = containerList.filter((item) =>
      item.index === index
    );
      ...
      <div className="App flex justify-items-center items-center flex-col w-full   " >
          ...
          <div className=" popup  w-3/12 items-center ">
            {selectedFolder && selectedFiles || selectedSpecificFile ? (
              ...
              <div className='flex flex-col items-center'>
                ...
                <label className='text-white text-center items-center my-4' htmlFor="fileInput"> Upload the folder to access its contents</label>
                <input
                  type="file"
                  id="fileInput"
                  webkitdirectory=""
                ...
                <label htmlFor="specificFileInput" className='text-white text-center items-center my-4'> Select a specific file</label>
...

        {
          ...
          < div className='w-full flex flex-wrap overflow-y-scroll scrollbar  scrollbar-thin  scrollbar-thumb-gray-900 scrollbar-track-gray-500 justify-between  p-3 '>
            ...
            {containerList && selectedOption === 'logs' || selectedOption === 'Inspectc' ? (
              <>
            ...
            <div className='flex flex-col justify-center items-center '>
              {importImg?.map((x, y) => (
                <div key={y} style={{ backgroundColor: '#17222A' }} className="flex w-11/12 items-center justify-between p-2 my-4 pointer-events">
                  ...
                  <button onClick={pullHandler} type="submit" className="text-white  right-2.5 bottom-2.5 current-pointer  hover:bg-gray-300 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-4 py-2 bg-gray-400  dark:focus:ring-gray-700">Pull Image</button>
                </div>
              ))}
            </div>
          </div>
...
const mapStateToProps = (state) => ({
  ...
  containerList: state.authReducer.containerList,
  listContainer: state.authReducer.listContainer,
  
  
})

const mapDispatchToProps = { geNetworkData, CreateNetwork,ImportImage,ContainerList,ListofImages }
